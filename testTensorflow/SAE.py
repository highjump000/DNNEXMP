...
def train():
    ...
    def displayImageTest(orig, blocking=False):
        ...
        for r in range(tile_size):
            for c in range(tile_size):
                index = r * tile_size + c
                patches[index] = orig[r * PATCHWIDTH:r * PATCHWIDTH + PATCHWIDTH,
                                 c * PATCHWIDTH:c * PATCHWIDTH + PATCHWIDTH].flatten()
        ...
        for r in range(tile_size):
            for c in range(tile_size):
                repro[r * PATCHWIDTH:r * PATCHWIDTH + PATCHWIDTH,
                c * PATCHWIDTH:c * PATCHWIDTH + PATCHWIDTH] = np.reshape(result[r * tile_size + c],
    ...
    def autoencoder(X, weights, biases):
        ...
        with tf.name_scope('output_layer'):
            out = tf.sigmoid(tf.add(tf.matmul(hiddenlayer, weights['out']),biases['out']))
