...
class MarketEnv(gym.Env):
	...
	def __init__(self, dir_path, target_codes, input_codes, start_date, end_date, scope = 60, sudden_death = -1., cumulative_reward = False):
		...
		self.scope = scope #distance
		self.sudden_death = sudden_death #?de
	...
	def _step(self, action):
		if self.done:
			return self.state, self.reward, self.done, {} #when it is done?

		self.reward = 0
		if self.actions[action] == "LONG":
			if sum(self.boughts) < 0: # if everythings is sell
	...
	def _reset(self):
		self.targetCode = self.targetCodes[int(random() * len(self.targetCodes))] # random code
	...
	def defineState(self):
		...
		budget = (sum(self.boughts) / len(self.boughts)) if len(self.boughts) > 0 else 1. #profit per trade ???
		size = math.log(max(1., len(self.boughts)), 100) # why log 100
		...
		for i in xrange(self.scope):
			try:
				subject.append([self.target[self.targetDates[self.currentTargetIndex - 1 - i]][2]]) # close info
				subjectVolume.append([self.target[self.targetDates[self.currentTargetIndex - 1 - i]][3]]) # vloumn info
			except Exception, e:
