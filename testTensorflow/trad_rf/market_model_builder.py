...
class MarketPolicyGradientModelBuilder(AbstractModelBuilder):
	...
	def buildModel(self):
		...
		model = Model(input = input, output = V)
...
class MarketModelBuilder(AbstractModelBuilder):
	...
	def buildModel(self):
		from keras.models import Model
		from keras.layers import merge, Convolution2D, MaxPooling2D, Input, Dense, Flatten, Dropout, Reshape, TimeDistributed, BatchNormalization, Merge, merge,Activation
		from keras.layers.advanced_activations import LeakyReLU
		...
		for i in xrange(1):
			...
			h = LeakyReLU(0.001)(h)
			'''
			h = Convolution2D(100,1, 30, border_mode = 'valid')(S)
			h = Activation('tanh')(h)
			h = Convolution2D(100,1, 20, border_mode = 'valid')(S)
			h = Activation('tanh')(h)
			h = Convolution2D(100, 1, 10, border_mode='valid')(S)
			h = Activation('tanh')(h)
			h = Flatten()(h)
			h = Dense(30)(h)
			h = Activation('tanh')(h)
			...
			#h = Convolution2D(2048, 1, 60, border_mode = 'valid')(S)
			#h = LeakyReLU(0.001)(h)
